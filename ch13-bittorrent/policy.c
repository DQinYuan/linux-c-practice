...
int select_optunchoke_peer()
{
	...
	ret = get_rand_numbers(count); // 按照一个随机的顺序去找
	if(ret < 0) {
		printf("%s:%d get rand numbers error\n",__FILE__,__LINE__);
		return -1;
	}
	while(i < count) {   // count是指peer总数
...
{
	Peer    *p       = peer_head;
	time_t  time_now = time(NULL);  // 以 second为单位的时间
	long    t        = 0;
	...
	while(p != NULL) {
		if(p->last_down_timestamp == 0) {
			...
			else  p->down_rate = p->down_count / t;  // 每 s 下载速率
			p->down_count          = 0;
		...
		if(p->last_up_timestamp == 0) {
			...
			if(t == 0)  printf("%s:%d time is 0\n",__FILE__,__LINE__);
			else  p->up_rate = p->up_count / t;  // 每 s 上传速率
			p->up_count          = 0;
...
int create_req_slice_msg(Peer *node)
{
	...
	if(p != NULL) {
		...
		// 当前piece还有未请求的slice,则构造对下一个slice的请求消息
	...
	if(i == pieces_length/20) {
		...
		for(i = 0; i < pieces_length/20; i++) { // 按顺序找一个我们没有的 piece 去请求
